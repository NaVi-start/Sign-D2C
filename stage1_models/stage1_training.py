...
from stage1_models.stage1_prediction import validate_on_data
...
class TrainManager_VAE:
    def __init__(self, model: PoseVAE, config: dict, src_vocab: Vocabulary, test=False):
        ...
        self.scheduler, self.scheduler_step_at = build_scheduler(
            ...
            hidden_size=config["model"]["PoseVAE"]["hidden_size"])
    ...
    def _train_batch(self, batch: Batch, update: bool = True) -> Tensor:
        ...
        skel_out = self.model(batch.trg_input[:, :, :150], batch.trg_mask)
        batch_loss = self.loss(skel_out, batch.trg_input[:, :, :150])
...
def train_stage1(cfg_file: str, ckpt=None) -> None:
    ...
    trainer = TrainManager_VAE(model=model, config=cfg, src_vocab=src_vocab)
    shutil.copy2(cfg_file, trainer.model_dir+"/config.yaml")
    log_cfg(cfg, trainer.logger)

    trainer.train_and_validate(train_data=train_data, valid_data=dev_data)

def test_stage1(cfg_file, ckpt: str) -> None:
    ...
    trainer = TrainManager_VAE(model=model, config=cfg, src_vocab=src_vocab, test=True)
